...
async def consume():
    ...
    # POST to the appropiate endpoint to create a consumer group
    # See: https://docs.confluent.io/current/kafka-rest/api.html#post--consumers-(string-group_name)
    resp = requests.post(
    ...
    # POST the subscription payload
    # See: https://docs.confluent.io/current/kafka-rest/api.html#consumers

    base_uri = str.replace(resp_data["base_uri"], "rest-proxy", "0.0.0.0")
    resp = requests.post(f"{base_uri}/subscription", data=json.dumps(data), headers=headers)
    try:
        resp.raise_for_status()
    except:
    ...
    while True:
        ...
        resp = requests.get(f"{base_uri}/records", headers=headers)
        try:
            resp.raise_for_status()
        except:
